	...
	void data(){
      ...
      if(vmpi::my_rank == 0){
         if(!zmag.is_open()){
            ...
            if(sim::load_checkpoint_flag && sim::load_checkpoint_continue_flag) zmag.open(std::getenv("OUTPUT_FILE"),std::ofstream::app);
            // otherwise overwrite file
            else{
               zmag.open(std::getenv("OUTPUT_FILE"),std::ofstream::trunc);
